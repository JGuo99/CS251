...
* Last Edit: 5/1/19
* Jeff Guo
* Authors Comment: I wasn't able to figure out the time test. Due to finals being around
* the corner I decided to just submit what I have so I have more time to study. Therefore,
* from parial score I am getting 130/150.
*/
#include <iostream>
#include <vector>
#include <queue>
...
class graph {
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == DISCOVERED){  //if next node is discovered
            //check if distance from current node to next node is shortest
            if(report[v].dist == report[u].dist + 1){
              report[v].npaths = report[u].npaths + report[v].npaths; //update npaths
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt,
      ...
      //When there is no path ends at dest
      if(rpt[dest].pred == -1){
        return false;
      }else if(rpt[dest].pred == dest){ //return true, when path is found at dest
        path.push_back(dest); //add node to path
        return true;
      }
      //Recursively add current predecessor
      extract_path(rpt, rpt[dest].pred, path);
      path.push_back(dest);
      return true;
////////////////1st Attemp: Same idea but doesn't work/////////////////////////////////
      // //succ (successor) to keep track of visited node
      // int succ;
      // //curr (current) to keep track of currently visited node
      // int curr = dest;
      //
      // //use do-while loop instead of while loop because we want to execute the bode atleast once
      // do{
      //   path.push_back(curr); //add current node to path
      //   succ = curr; //set current node as successor
      //   curr = rpt[curr].pred; //update current to predecessor
      // }while(succ != curr); //loop till current node is equal to src node
      // return true;
    }
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      //Setup rpt dist, pred and find starting nodes
      ...
      for(int i = 0; i < t.size(); i++){
        u = t[i];
        //edge outgoing from u
        for(edge &e: vertices[u].outgoing){
          int v = e.vertex_id;
          if(rpt[v].dist < rpt[u].dist + e.weight){ //When new vertex is smaller than u dist and edge
            rpt[v].dist = rpt[u].dist + e.weight; //Set new vertex to u dist + edge
            rpt[v].pred = u;  //u then becomes v pred
          }
        }
      }return true;
    }
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
      ...
      //find the numbers of outgoing edge
      for(int i = 0; i < num_nodes(); i++){
        if(vertices[i].incoming.size() == 0){
          outpaths(i, rpt);
        }
      }
      //find the number of incoming edges
      for(int i = 0; i < num_nodes(); i++){
        if(vertices[i].outgoing.size() == 0){
          inpaths(i, rpt);
        }
      }
      //add (multiply in this case) both in and out going paths
      for(int i = 0; i < num_nodes(); i++){
        rpt[i].npaths = rpt[i].ingo * rpt[i].outgo;
      }
      return true;
    }

  private:
    //Since a path can be outgoing or in going we need two helper functions
    int outpaths(int src, vector<vertex_label> &rpt){
      int osize = vertices[src].outgoing.size();
      if(osize == 0){
        rpt[src].outgo = 1;
      }else{
       for(int i = 0; i < osize; i++){  //look at the edges
         int j = vertices[src].outgoing[i].vertex_id;
         if(rpt[j].outgo > 0){
           rpt[src].outgo = rpt[src].outgo + rpt[j].outgo;
           rpt[src].npaths = rpt[src].outgo;
         }else{
           rpt[src].outgo = rpt[src].outgo + outpaths(j, rpt);
         }
       }
      }
      return rpt[src].outgo;
    }

    int inpaths(int src, vector<vertex_label> &rpt){
      int isize = vertices[src].incoming.size();
      if(isize == 0){
        rpt[src].ingo = 1;
      }else{
       for(int i = 0; i < isize; i++){  //look at the edges
         int j = vertices[src].incoming[i].vertex_id;
         if(rpt[j].ingo > 0){
           rpt[src].ingo = rpt[src].ingo + rpt[j].ingo;
           rpt[src].npaths = rpt[src].ingo;
         }else{
           rpt[src].ingo = rpt[src].ingo+ inpaths(j, rpt);
         }
       }
      }
      return rpt[src].ingo;
  ...
  public:
    ...
    bool valid_topo_order(const vector<int> & order) {
      ...
      std::vector<bool> visited = vector<bool> (num_nodes());
      int vsize = visited.size();

      //initally set everything to flase (unvisited)
      for(int i = 0; i < vsize; i++){
        visited[i] = false;
      }
      for(int i = 0; i < order.size(); i++){
        int tmp = order[i];
        int isize = vertices[tmp].incoming.size();
        //check if vsize is a start node
        if(isize > 0){
          for(int j = 0; j < isize; j++){
            //This will make sure that it return false, if they are not returning false.
            if(visited[vertices[tmp].incoming[j].vertex_id] == false){
              return false;
            }
          }
        }
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      //to store path from vector
      string path;
      path.clear(); //this clears path NOT path"s"
      for(int i = 0; i < vertices.size(); i++){
        if(vertices[i].incoming.size() == 0){
          epathsHelper(paths, path, target, i);
        }
      }return true;
    }
  private:
    void epathsHelper(vector<string> &paths, string pa, int tar, int i){
      int j;
      bool vaild = false; //initally set vaild to false
      //base case, check if tar (target) is found
      if(i == tar){
        paths.push_back(pa + vertices[i].name); //add to path
      }else{
        for(edge &e : vertices[i].outgoing){  //loop through the edges
          j = e.vertex_id;
          if(vaild == false){
            pa += (vertices[i].name + " ");
          }
          epathsHelper(paths, pa, tar, j);
          vaild = true;
        }
      }
    }
